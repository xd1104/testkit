...
async function collectSites(params, onProgress) {
  ...
  const mainProvs = mainRes.providers;
  const testProvs = testRes.providers;
  const mainOrder = mainRes.categoryOrder; // { 種類: [canon 依主網順序] }
  const testOrder = testRes.categoryOrder;
  ...
  return { mainHost, testHost, mainProvs, testProvs, mainByCanon, testByCanon, mainOrder, testOrder, common, games };
}
